...
# This is also where all of the data saving function is carried out                     #
#########################################################################################
  ...
  observeEvent(input$displayTeams2, {
    observeEvent(input$dataTypesSingle, {
      observeEvent(input$dataTypesSingle4388, {
        ...
        #this combines all of the rows from both red and blue and makes one global var
        ...
        try(graphData <-
          displayData2 %>% select(all_of(dataTypes2), Match.. , Team..))
        try(graphData <- meltData(graphData, dataTypes2))
        
        #This is where the data is being filtered based on the slicer selection
        #on the dashboard page, and creates the display data set
        try(graphData4388 <-
          displayData4388 %>% select(all_of(dataTypes4388), Match.., Team..))
        ...
        try(allTeams2 <- ddply(
          ...
          .(Team..),
          summarise,
          Red.Card = -1*mean(replace_na(as.logical(Red.Card), FALSE), na.rm = TRUE),
          Yellow.Card = -1*mean(replace_na(as.logical(Yellow.Card), FALSE), na.rm = TRUE),
          Fouls = -1*mean(Fouls, na.rm = TRUE),
          Tech.Fouls = -1*mean(Tech.Fouls, na.rm = TRUE),
          Disabled = -1*mean(replace_na(as.logical(Disabled), FALSE), na.rm = TRUE),
          Flipped.Over = -1*mean(replace_na(as.logical(Flipped.Over), FALSE), na.rm = TRUE),
          Inner.Port = mean(Inner.Port, na.rm = TRUE),
          Outer.Port = mean(Outer.Port, na.rm = TRUE),
          Lower.Port = mean(Lower.Port, na.rm = TRUE),
          Assisted.Climb = mean(replace_na(as.logical(Assisted.Climb), FALSE), na.rm = TRUE),
